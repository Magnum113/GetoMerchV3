...
`/api/admin/ozon/orders` использует аналогичный bounded `offset` contract.
Клиент дочитывает полный набор сохранённых заказов перед распределением
остатков по срочности, поэтому более старый активный FBS-заказ не выпадает из
резервирования из-за лимита истории.

`/api/admin/finance/ozon` и `/api/admin/expenses` также возвращают
`offset/nextOffset/hasMore`. Аналитический dashboard запрашивает объединённое
date-window текущего и предыдущего периода, дочитывает все его страницы и
отбрасывает устаревший ответ при быстром переключении периода.
