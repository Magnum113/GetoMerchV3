...
- **Фича-флаги** — владелец включает готовые бизнес-контуры в разделе
  `Настройки`; выключенная функция скрывается из интерфейса и закрывается на
  уровне server API и фоновых обработчиков.
...
Пользовательский контур маркировки дополнительно управляется флагом
`chestny_znak` в `Настройки -> Функции`. Состояние хранится в
`merch_admin_feature_flags` локальной production-БД и по умолчанию выключено.
В выключенном состоянии скрываются раздел и действия с КМ в заказах, marking
API отвечает fail-closed, Ozon orders не гидрируются marking read models,
обычная складская отгрузка не создаёт handover/вывод из оборота, marking worker
не берёт новые задания, а Mac signer-agent не получает новые запросы на
подпись. История и уже зафиксированные юридически значимые события остаются
неизменными. Этот флаг не заменяет server-only env gates: даже после его
включения внешние операции отдельно разрешаются через
`GETOMERCH_MARKING_*_ENABLED` и allow-lists.
...
- Финансовые операции Ozon и расходы загружаются bounded-страницами. Дашборд
  передаёт в PostgreSQL окно выбранного и предыдущего периода, поэтому годовая
  аналитика не обрезается первыми 1000 операциями и не читает лишнюю историю.
