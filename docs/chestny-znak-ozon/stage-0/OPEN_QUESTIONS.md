...
| Блокирует ли отсутствие РД технический flow? | Нет; поле хранится только как справочная характеристика и не является readiness/canary gate | решение владельца бизнеса |
...
Отсутствие РД и значения `goodMarkFlag`/`goodTurnFlag` могут отображаться для
диагностики, но не участвуют в автоматическом запрете заказа КМ, нанесения,
ввода в оборот или передачи кода Ozon.
