...
Статус: migration `0014`, credentials, nginx и remote transport установлены.
Heartbeat с реальным Рутокеном, физическая attached CAdES-BES подпись и
auth-only production True API canary проверены. Действующая лицензия CryptoPro
CSP установлена; см.
...
На Mac работают два отдельных процесса:
...
2. `marking:mac-agent` сам подключается к `admin.komui.ru`, получает одну
   задачу и передаёт её локальному signer. Штатный ручной launcher запускает
   агент в фоне только на время foreground-сессии signer.
...
Основной ручной запуск выполняется одной командой в обычном Terminal:

```bash
cd /Users/kadimagomedov/Documents/GetoMerchV3
ops/chestny-znak/run-mac-signing-session
```

Launcher читает защищённый `marking-mac-agent.env`, запускает Mac-agent в фоне
и оставляет signer в текущем Terminal. PIN вводится скрыто один раз при старте
и остаётся только в очищаемом буфере памяти signer до остановки процесса.
Каждый дочерний `cryptcp` получает PIN через stdin с `-askpin`: PIN отсутствует
в env, argv, файлах, логах и сетевых запросах. После `Ctrl+C` или завершения
signer PIN очищается, а фоновый агент также останавливается. Его безопасный
технический журнал хранится в
`~/.config/getomerch-marking/logs/mac-agent.log` с правами `0600`.

PIN не нужно и нельзя передавать в чат, env, аргументы команды или сервер.
При ошибке PIN сессия блокируется без автоматических повторов: signer нужно
остановить, запустить заново и один раз ввести правильный PIN. Это защищает
Рутокен от серии ошибочных попыток.

### Раздельный диагностический запуск

Для раздельной диагностики процессы можно запустить вручную в двух терминалах.
Signer сам запросит PIN один раз при запуске.
...
- **PIN:** signer запрашивает его один раз при старте до публикации Unix
  socket. UI показывает `Сессия разблокирована`. При ошибке PIN буфер
  очищается, запрос завершается без автоматического повтора, а UI просит
  перезапустить signer. Ожидание remote broker составляет до пяти минут, чтобы
  краткий разрыв связи не создавал новый CRPT challenge.
- **Ответ потерян после подписи:** одинаковый `purpose + payload SHA-256 +
  requested_by` повторно использует существующий pending/leased/signed broker
  request. Новый CRPT challenge имеет другой hash и требует новой подписи, но
  использует уже разблокированную локальную PIN-сессию.
...
3. Действующая лицензия CSP установлена; физическая подпись и получение
   production auth token успешно проверены.
