...
Heartbeat с реальным Рутокеном проверен. Действующая лицензия CryptoPro CSP
установлена; физическая подпись и auth canary после установки еще не выполнены;
см.
...
3. Действующая лицензия CSP установлена; физическую подпись и получение auth
   token нужно повторить отдельным canary.
