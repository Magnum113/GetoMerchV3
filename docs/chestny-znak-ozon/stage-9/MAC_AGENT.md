...
и оставляет signer в текущем Terminal с интерактивным stdin. При запросе
подписи PIN вводится только в этом Terminal. После `Ctrl+C` или завершения
signer фоновый агент также останавливается. Его безопасный технический журнал
хранится в `~/.config/getomerch-marking/logs/mac-agent.log` с правами `0600`.
...
- **Нужен PIN:** CryptoPro запрашивает его на Mac; UI показывает
  ...
  отведено до 60 секунд; Unix socket ожидает ответ 75 секунд, клиент агента —
  80 секунд, а lease broker продлевается signing-heartbeat до завершения.
