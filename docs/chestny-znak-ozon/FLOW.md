...
- проверку классификации товара и применимых разрешительных документов;
...
Отсутствие РД и диагностические флаги НК не блокируют product readiness,
заказ/импорт КМ, назначение, печать и физическое нанесение. Однако реальный
canary 14 августа 2026 года подтвердил, что реквизиты документа соответствия
обязательны в `certificate_document_data` для принятия
`LP_INTRODUCE_GOODS`. Поэтому перед вводом в оборот действует отдельный
fail-closed gate: вид, номер и дата РД должны быть проверены и сохранены.
...
4. Product readiness: GTIN, НК, реквизиты РД для будущего ввода и UI.
...
На 14 августа 2026 года этапы 0-13 реализованы. После rehearsal миграции
`0005`-`0020` применены к `getomerch_production`, серверные marking credentials
...
Этап 14 начат после первого реального `LP_INTRODUCE_GOODS`: документ был
подписан и отправлен, но отклонён из-за отсутствующих реквизитов РД. Товар уже
передан Ozon без подтверждённого `in_circulation` и без принятого exemplar,
поэтому этот опыт зафиксирован как инцидент, а не успешный canary. Новый
контрольный `LP_INTRODUCE_GOODS`, подтверждённые `in_circulation`, Ozon
exemplar, `LK_RECEIPT/DISTANCE`, `LP_RETURN` и физическая проверка шаблона на
целевых принтерах остаются обязательными воротами перед rollout.

## 33. Материалы, которые нужно получить от владельца

1. Выгрузка НК по GTIN всех размеров и цветов.
2. ТН ВЭД/ОКПД2; РД можно предоставить позднее относительно настройки
   профиля, но обязательно до первого `LP_INTRODUCE_GOODS`.
