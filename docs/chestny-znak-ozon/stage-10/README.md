...
Привязка разрешена только при совпадении типа, нормализованной товарной группы
и SHA-256 исходного содержимого документа. `senderInn` сверяется с рабочим
участником, если True API вернул это необязательное поле; ИНН внутри документа
защищён точным hash ранее подписанного локального payload. Автоматический
повторный submit до сверки запрещён. UI обновляет результат фонового задания
каждые 10 секунд.
