...
Дата актуализации: 18 августа 2026 года.
...
В админке добавлен бизнес-флаг `chestny_znak`, хранящийся в локальной
PostgreSQL. До появления применимого документа соответствия он выключен. Это
верхний fail-closed gate для всего пользовательского контура; узкие env-флаги
внешних операций остаются отдельными предохранителями.
...
Состояние доступности пользовательских функций хранится отдельно в
`public.merch_admin_feature_flags`, безопасно читается через
`getomerch_admin.feature_flag_safe` и изменяется только аудируемой
`SECURITY DEFINER`-функцией. Прямой DML роли `getomerch_app` к таблице флагов
запрещён.

## 2.1. Бизнес-флаг интеграции

Флаг `chestny_znak` меняется в `Настройки -> Функции` и по умолчанию равен
`false`.

При выключении:

- пункт `Честный знак`, его страницы и все элементы КМ в заказах скрыты;
- `/api/admin/marking/*` закрыт fail-closed;
- список Ozon-заказов не загружает assignment/candidate read models;
- обычная отгрузка Ozon не создаёт marking gate, handover или документ вывода;
- marking worker не получает новые задания;
- Mac signer-agent сохраняет heartbeat и может завершить уже взятый запрос,
  но не получает новый запрос на подпись;
- сохранённые КМ, документы, события и аудит не удаляются и не изменяются.

Если флаг выключен во время уже начатой внешней операции, активная атомарная
операция не обрывается посередине: она завершается до согласованного состояния,
после чего новые задания не выдаются. Это предотвращает неопределённый статус
в ГИС МТ или Ozon.

Включение `chestny_znak` открывает только бизнес-интерфейс. Оно не включает
автоматически импорт, подпись, ГИС МТ, СУЗ, Ozon write, ввод, вывод или возврат:
каждый такой канал по-прежнему требует своего server-only env-флага,
allow-list и готовых обязательных реквизитов.
