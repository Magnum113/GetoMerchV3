...
FBS-заказу и проверен в этикетке 58x40. Все внешние write-операции маркировки
выключены. Миграции fulfillment/marking `0005`-`0019` развернуты в production.
...
GTIN, тип эмиссии и документы ГИС МТ. Сведения о РД могут храниться справочно,
но их отсутствие не блокирует marking flow.
...
Отсутствие РД и диагностические флаги НК не являются техническим gate.
...
1. Проверить производственную модель, GTIN и актуальные API-контракты из
   личных кабинетов; сведения о РД учитывать только справочно.
...
4. Добавить trade items НК/GTIN, product profiles, необязательные метаданные
   РД и расхождения с Ozon.
...
- коды ТН ВЭД/ОКПД2; сведения о РД можно добавить позднее как необязательные
  метаданные;
