...
OZON_CLIENT_ID=...
OZON_API_KEY=...
# временный fallback, пока в коде еще встречается старое имя:
OZON_CLIEN_ID=...
...
Сейчас проект `GetoMerchV3` использует `npm` и `package-lock.json`, поэтому
первый deploy должен идти через `npm ci`. На `pnpm` можно переходить только
отдельным изменением с добавлением `pnpm-lock.yaml` и обновлением deploy
script. Не смешивать npm/pnpm/yarn без причины.
...
ExecStart=/usr/bin/npm start -- --hostname 127.0.0.1 --port 3100
Restart=always
```

Если позже проект перейдет на `pnpm`, тогда `ExecStart` можно заменить на
`/usr/bin/pnpm start -- --hostname 127.0.0.1 --port 3100`.
