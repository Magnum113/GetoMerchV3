...
export async function reconcileCrptIntroduction(
  ...
  return withServerDatabaseTransaction(async (query) => {
    const local = (await query<{
      ...
      `SELECT status, error_code, external_document_id
       FROM getomerch_marking.document_safe
    ...
    const queued = await enqueueJob({
      type: "marking_crpt_document_poll",
      dedupeKey: `crpt-introduction-reconcile:${input.documentId}:${input.externalDocumentId}`,
      idempotencyKey: context.idempotencyKey,
      payload: {
        reconcileDocumentId: input.documentId,
