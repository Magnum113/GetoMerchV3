...
const checks = [
  ...
  { name: "Ozon orders page data with valid cookie returns 200", run: () => expectStatus("/api/admin/ozon/orders?limit=10", 200, authCookie()) },
  { name: "inventory page data with valid cookie returns 200", run: () => expectStatus("/api/admin/inventory?limit=50", 200, authCookie()) },
